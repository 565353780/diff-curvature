...
from diff_curvature.Method.pcl_geometry import disambiguate


class DiffGlobalPC(nn.Module):
    def __init__(
        self, pointscloud, k=20, normals_field=None, knn_info=None, normals=None
    ):
        super().__init__()
        self.k = k
        ...
        if knn_info is None:
            if normals_field is not None:
                self.knn_info = knn_points(
                    torch.cat([pointscloud, normals_field * 0.02], dim=-1),
                    torch.cat([pointscloud, normals_field * 0.02], dim=-1),
                    K=k,
                    return_nn=True,
                )
                ...
                self.knn_info = knn_points(
                    pointscloud, pointscloud, K=k, return_nn=True
                )
        else:
            self.knn_info = knn_info

        self.frames_field = self.frames_field_from_local_statistics(
            pointscloud, self.knn_info
        )
        ...
        if normals is not None:
            ...
            self.frames_field[:, :, 1, :] = (
                self.frames_field[:, :, 1, :]
                - (self.frames_field[:, :, 1, :] * self.frames_field[:, :, 0, :]).sum(
                    -1, keepdim=True
                )
                * self.frames_field[:, :, 0, :]
            )
            self.frames_field[:, :, 1, :] = self.frames_field[
                :, :, 1, :
            ] / self.frames_field[:, :, 1, :].norm(dim=-1, keepdim=True)
            self.frames_field[:, :, 2, :] = torch.cross(
                self.frames_field[:, :, 0, :], self.frames_field[:, :, 1, :]
            )
    ...
    def frames_field_from_local_statistics(self, pointscloud, knn_info=None):
        ...
        knn_points_centered = knn_info.knn[..., :3] - knn_info.knn[..., :3].mean(
            -2, keepdim=True
        )
        covs_field = torch.matmul(
            knn_points_centered.transpose(-1, -2), knn_points_centered
        ) / (knn_points_centered.shape[-1] - 1)
        frames_field = torch.linalg.eigh(covs_field).eigenvectors.transpose(
            -1, -2
        )  # B x N x 3 x 3

        # disambiguate the normals
        frames_field[:, :, 0, :] = disambiguate(
            frames_field[:, :, 0, :], pointscloud, K=self.k
        )

        frames_field[:, :, 1, :] = frames_field[
            :, :, 1, :
        ] * frames_field.det().unsqueeze(-1)

        return frames_field

    def local_voronoi_area(
        self, frames_field: torch.Tensor = None, local_W: int = 128, n_temp: int = 2000
    ):
        """
        ...
        X, Y = torch.meshgrid(
            torch.linspace(-1, 1, local_W), torch.linspace(-1, 1, local_W)
        )
        ...
        local_pt_difference = (
            knn_info.knn[..., :3] - self.pointscloud[:, :, None, :]
        )  # B x N x K x 3
        ...
        local_dpt_tangent1 = (local_pt_difference * tangent1_field[:, :, None, :]).sum(
            -1, keepdim=True
        )  # B x N x K x 1
        local_dpt_tangent2 = (local_pt_difference * tangent2_field[:, :, None, :]).sum(
            -1, keepdim=True
        )  # B x N x K x 1
        local_dpt_tangent = torch.cat(
            (local_dpt_tangent1, local_dpt_tangent2), dim=-1
        )  # B x N x K x 2

        # ----------------- local exclusive voronoi area ----------
        local_rescalar = 1.1

        local_knn_bbox_tangent = (
            torch.stack(
                (
                    local_dpt_tangent.min(dim=-2).values,
                    local_dpt_tangent.max(dim=-2).values,
                ),
                dim=-1,
            )
            * local_rescalar
        ...
        local_knn_bbox_tangent_length = (
            local_knn_bbox_tangent[..., 1:] - local_knn_bbox_tangent[..., 0:1]
        )

        local_knn_coord_tangent = (
            local_dpt_tangent - local_knn_bbox_tangent[..., 0:1].transpose(-1, -2)
        ) / local_knn_bbox_tangent_length.max(-2, keepdim=True).values.repeat(
            1, 1, 1, 2
        ) * 2 - 1  # B x N x K x 2

        local_closest_idx = knn_points(
            points_meshgrid.repeat(N * B, 1, 1),
            local_knn_coord_tangent.view(-1, k, 2),
            K=1,
        ).idx  # (B*N) x K x 1

        local_center_voronoi = torch.where(local_closest_idx == 0)

        local_center_voronoi = scatter(
            torch.ones_like(local_center_voronoi[0]),
            local_center_voronoi[0],
            dim_size=N * B,
        )

        local_center_voronoi = repeat(
            local_center_voronoi, "(b n) -> b n 1", b=B, n=N
        )  # B x N

        voronoi_area_list = (
            local_center_voronoi
            * (local_knn_bbox_tangent_length.max(-2).values ** 2)
            / (local_W - 1) ** 2
        )
    ...
    def weingarten_fields(
        self, frames_field: torch.Tensor = None, if_strict: bool = False
    ):
        """
        ...
        # local normals difference
        local_normals_difference = knn_gather(
            normals_field, knn_info.idx
        ) - normals_field.unsqueeze(-2)  # B x N x K x 3
        ...
        local_dnormals_tangent1 = (
            local_normals_difference * tangent1_field[:, :, None, :]
        ).sum(-1, keepdim=True)
        local_dnormals_tangent2 = (
            local_normals_difference * tangent2_field[:, :, None, :]
        ).sum(-1, keepdim=True)
        local_dnormals_tangent = torch.cat(
            (local_dnormals_tangent1, local_dnormals_tangent2), dim=-1
        )  # B x N x K x 2
        ...
        if if_strict == False:
            Weingarten_fields = torch.matmul(
                YXT, torch.inverse(XXT + 1e-8 * torch.eye(2).type_as(XXT))
            )
            Weingarten_fields = (
                Weingarten_fields + Weingarten_fields.transpose(-1, -2)
            ) / 2
    ...
    def differentiable_euler_number(
        self,
        frames_field: torch.Tensor = None,
        local_W: int = 64,
        n_temp: int = 2000,
        return_area=False,
        return_gaussian_curvature=False,
        ...
        # print(euler_number)
        gaussian_curvature_pcl = self.gaussian_curvature(
            frames_field, if_strict=if_strict
        )
        euler_number = (
            (gaussian_curvature_pcl.view(B, -1) * area.view(B, -1)).sum(-1) / np.pi / 2
        )
